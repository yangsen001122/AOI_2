...
int CppSQLite3Query::fieldIndex( char* szField)
{
	checkVM();
...
bool CppSQLite3DB::CreateUserView(char* dbName)
{
	bool bRet = false;
	//char sql[3000];
	char *pErrMsg = 0;
	//sprintf_s(sql, SQL_CREATE_VIEW);
	int rc = sqlite3_exec(m_ptrSqlite3, SQL_CREATE_VIEW, 0, 0, &pErrMsg);
	if (rc != SQLITE_OK)
	{
...
int CppSQLite3DB::getRowNew(const char* szSQL)
{
	checkDB();
...
QUERY_USER_RESULT CppSQLite3DB::getUserResult(int m_Row)
{
	checkDB();
	int index;
	char* szError = 0;
	...
	int nRows(0);
	int nCols(0);
	QUERY_USER_RESULT m_QueryResult = { 0,"","","" };
	int nRet = sqlite3_get_table(m_ptrSqlite3, SQL_QUERY_USER_MGMT_DATA, &paszResults, &nRows, &nCols, &szError);
	if (nRet == SQLITE_OK)
	{
		index = nCols;
		int order = 0;
		order = (m_Row + 1)*index;
		m_QueryResult.id = atoi(paszResults[order]);
		m_QueryResult.username = paszResults[order + 1];
		m_QueryResult.key = paszResults[order + 2];
		m_QueryResult.level = paszResults[order + 3];
...
QUERY_USER_RESULT CppSQLite3DB::getPassWord(const char* username)
{
	checkDB();
	char   sql[1024];
	char* szError = 0;
	...
	int nCols(0);
	QUERY_USER_RESULT m_QueryResult = { 0,"","","" };	
	sprintf_s(sql, SQL_QUERY_USER_MGMT_ROW, username);
	int nRet = sqlite3_get_table(m_ptrSqlite3, sql, &paszResults, &nRows, &nCols, &szError);
	if (nRet == SQLITE_OK)
	{
		int order = nCols;
		{
...
int CppSQLite3DB::deleteUserRow(int id)
{
	checkDB();
	char   sql[1024];
	char* szError = 0;
	sprintf_s(sql, SQL_DELETE_USER_MGMT_ROW, id);
	int nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
		
	}
	return nRet;
}

int CppSQLite3DB::getRow(const char* field)
{
	checkDB();
	int Row = 0;
	char* szError = 0;
	...
	int nCols(0);
	char   sql[1024];
	sprintf_s(sql, SQL_QUERY_USER_MGMT_ROW, field);
	int nRet = sqlite3_get_table(m_ptrSqlite3, sql, &paszResults, &nRows, &nCols, &szError);
	if (nRet == SQLITE_OK)
	{
		Row=nRows;
...
int CppSQLite3DB::getRecordRow(const char*field, const char* crtProj)
{
	checkDB();
	int Row = 0;
	char* szError = 0;
...
int CppSQLite3DB::deleteRecord(const char*field, const char* crtProj)
{
	checkDB();
	char   sql[1024];
	char* szError = 0;
	sprintf_s(sql, SQL_DELETE_RECORD, crtProj, field);
	int nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
...
int CppSQLite3DB::saveUserData(const char* tbname, const char* username, const char*key,const char*level)
{
	char   sql[1024];
	char* szError = 0;
	int nRet=0;
	sprintf_s(sql, SQL_INSERT_USER_MGMT_DATA, tbname,username, key, level);
	nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
		throw CppSQLite3Exception(nRet, szError);
...
int CppSQLite3DB::saveUserOPT(const char* tbname, const char* username, const char*opt, const char*time)
{
	char   sql[1024];
	char* szError = 0;
	int nRet = 0;
	sprintf_s(sql, SQL_INSERT_USER_OPT_DATA, tbname, username, opt, time);
	nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
...
int CppSQLite3DB::saveCountDataInstant(const char* tbname, const char* date, const char*total, const char*OKCount, const char*OKCountRate, const char* Time)
{
	char   sql[1024];
	char* szError = 0;
	int nRet = 0;
	sprintf_s(sql, SQL_INSERT_COUNT_DATA, tbname, date, total, OKCount, OKCountRate, Time);
	nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
...
int CppSQLite3DB::updateUserPassWord(const char*username,const char*key,const char*level,int id)
{
	char   sql[1024];
	char* szError = 0;
	int nRet = 0;
	sprintf_s(sql, SQL_UPDATE_USER_DATA, username, key, level,id);
	nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
...
int CppSQLite3DB::updateUserPassWord(const char*username, const char*level, int id)
{
	char   sql[1024];
	char* szError = 0;
	int nRet = 0;
	sprintf_s(sql, SQL_UPDATE_USER_DATANOKEY, username, level, id);
	nRet = sqlite3_exec(m_ptrSqlite3, sql, 0, 0, &szError);
	if (nRet != SQLITE_OK)
	{
		sqlite3_close(m_ptrSqlite3);
