...
void CLabelCtrl::OnPaint() 
{
	...
	{
		y = int((rect.Height() - tm.tmHeight) / 2);
	}
...
void CLabelCtrl::OnRButtonDblClk(UINT nFlags, CPoint point)
{
...
void CLabelCtrl::SetMainUiPtr(CWnd * ptrMainUi)
{
	m_ptrMainUi = ptrMainUi;
}

void CLabelCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if (m_ptrMainUi!=nullptr)
	{
		::PostMessage(m_ptrMainUi->m_hWnd, WM_SEND_MSG_LABELCTRL_RBTNDOWN, 0, (LPARAM)m_nID);
	}
	//int nid = nID;
